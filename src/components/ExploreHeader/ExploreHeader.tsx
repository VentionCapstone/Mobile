...
const ExploreHeader = () => {
  ...
  const [locationText, setLocationText] = useState(
    filter.location ? filter.location : DEFAULT_LOCATION
  );
  const [durationText, setDurationText] = useState(
    filter.checkInDate && filter.checkOutDate
      ? getStayDuration(filter.checkInDate, filter.checkOutDate)
      : DEFAULT_DURATION
  );
        ...
        <TouchableOpacity onPress={() => setSearchModalVisible(true)}>
          <ThemedView style={[styles.searchBar, LEVEL_1, colors && styles.darkModeShadow]}>
            ...
            <View style={styles.searchContent}>
              <Text style={styles.searchHeader}>{t('Where to go?')}</Text>
              <Text>
